...
{
    internal class StartsWithIgnoreCaseDirective : IFilterDirective
    {
        public string FilterSyntax
        {
            get
            {
                return Constants.ComparisonOperator.StartsWithIgnoreCase;
            }
        }

        public Expression GenerateExpression(ref MemberExpression property, ConstantExpression value)
        {
            var function = typeof(MiscExtensions).GetMethod(nameof(MiscExtensions.StartsWithIgnoreCase),
            ...
            if (function == null)
                throw new Exception($"{nameof(MiscExtensions.StartsWithIgnoreCase)} function is not found");
